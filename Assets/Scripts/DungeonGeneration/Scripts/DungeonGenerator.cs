    ...
    public class DungeonGenerator : MonoBehaviour
    {
        ...
        private IEnumerator CreateStartRoom()
        {
            ...
            for (int i = 1; i < dungeonConfig.maxRooms; i++)
            {
                ...
                var room = new RoomNode(position, null, $"room_{i}_{position.x}_{position.y}");
                generatedRooms.Add(room);
            }

            // Добавляем сгенерированные комнаты в общий список
            foreach (var room in generatedRooms)
            {
                roomNodes.Add(room);
                Debug.Log($"[DungeonGenerator] Создана комната типа {room.RoomType?.typeName ?? "null"} на позиции {room.Position}");
            }

            // Назначаем типы комнат после генерации
            AssignRoomTypes();
        ...
        {
            // Теперь назначение типа босс-комнаты происходит в AssignRoomTypes, поэтому здесь ничего не делаем
            yield return null;
        }
        ...
        private IEnumerator MovePlayerAndDestroyRoom(GameObject player, Transform spawnPoint, RoomNode currentRoom, string nextRoomId)
        {
            ...
            {
                Debug.LogWarning($"[DungeonGenerator] Предупреждение: игрок не был точно перемещён! Текущая позиция: {player.transform.position}, целевая позиция: {spawnPoint.position}, расстояние: {distance}");
                ...
                {
                    Debug.LogWarning($"[DungeonGenerator] Предупреждение: игрок не был точно перемещён даже после повторной попытки! Расстояние: {distance}. Старая комната не будет уничтожена.");
                }
            }
        }
        ...
        private IEnumerator LoadRoomCoroutine(string nextRoomId)
        {
            ...
            // Проверяем, не является ли комната уже посещенной
            if (nextRoom.RoomInstance != null)
            {
                Debug.LogError($"[DungeonGenerator] Попытка телепортации в уже посещенную комнату {nextRoomId}!");
                yield break;
            }
            ...
            // Проверяем, не пытаемся ли мы телепортироваться в текущую комнату
            if (currentRoom.Id == nextRoomId)
            {
                Debug.LogError($"[DungeonGenerator] Попытка телепортации в текущую комнату {nextRoomId}!");
                yield break;
            }

            // Если комната уже создана, просто активируем её и перемещаем игрока
            if (nextRoom.RoomInstance != null)
            {
            ...
            // Активируем комнату и перемещаем игрока
            yield return StartCoroutine(ActivateRoom(nextRoom));
            yield return StartCoroutine(MovePlayerToNewRoom(nextRoom, player, currentRoom));
        }
        ...
        private RoomNode FindStartRoom() => roomNodes.FirstOrDefault(r => r.RoomType == dungeonConfig.startRoomType);

        // Возвращает босс-комнату
        public RoomNode GetBossRoom() => roomNodes.FirstOrDefault(r => r.RoomType == dungeonConfig.bossRoomType);

        // Возвращает самую дальнюю комнату от стартовой
        private RoomNode FindFurthestRoom(RoomNode fromRoom)
        {
            ...
            foreach (var room in roomNodes)
            {
                ...
                float distance = Vector2.Distance(fromRoom.Position, room.Position);
